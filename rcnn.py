...
class GeneralizedRCNN(nn.Module):
    ...
    def inference(
        ...
        for i in range(len(koord)):
            koord[i][0]=koord[i][0]*ratio2 - 50
            koord[i][1]=koord[i][1]*ratio1
            koord[i][2]=koord[i][2]*ratio2 + 50
            koord[i][3]=koord[i][3]*ratio1 + 50
            propabilityTest.append(10.0)
        #print(koord)
        testBoundingBox=Boxes(koord)
        propabilityTest=torch.tensor(propabilityTest)
        #print(propabilityTest)
        
        test=Instances(groeße)
        test.__setattr__("proposal_boxes", testBoundingBox) 
        test.__setattr__("objectness_logits", propabilityTest)
        proposals=[test]
        #print (proposals)
